...
FrontEnd::FrontEnd(const string& next, const string& image,
  unsigned long duration) : Level("", next), m_image(nullptr), m_start(0),
  m_duration(duration){
...
bool FrontEnd::onMouseButtonEvent(const MouseButtonEvent& event){
...
bool FrontEnd::onJoyStickEvent(const JoyStickEvent& event){
...
bool FrontEnd::onKeyboardEvent(const KeyboardEvent& event){
