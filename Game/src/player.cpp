...
class Player::Impl {
    ...
    void get_key() {
        ...
        m_key = true;
        double x_axis = (double)env->canvas->w() * 1/35 + size + 2;
        double y_axis = (double)env->canvas->h() * 25/30 + size + 2;
        Item* key = new Item(m_player, "icon_key", "res/items/thumb.key.png",
                            x_axis, y_axis, 9999, true);
    ...
    void use_pill() {
        ...
        cout << "usou uma pilula! Recuperou " << recover << " de vida." << endl;
            ...
            cout << "usou uma pilula! Recuperou " << recover
            cout << " de sanidade." << endl;
...
const pair<double, double>& Player::moviment() const {
    return m_impl->moviment();
}

void Player::set_moviment(double xaxis, double yaxis) {
    m_impl->set_moviment(xaxis, yaxis);
}

void Player::set_current(string nova, int posx, int posy) {
    m_impl->set_current(nova, posx, posy);
}

void Player::set_strength(double strength) {
...
void Player::set_sanity(double sanity) {
    m_impl->set_sanity(sanity);
}

void Player::set_stamina(double stamina) {
...
void Player::set_key(bool input) {
...
void Player::show_stamina() {
    m_impl->show_stamina();
}

void Player::take_item() {
    m_impl->take_item();
}

void Player::jump_level() {
    m_impl->jump_level();
}

void Player::open_door() {
    m_impl->open_door();
}

void Player::push_item() {
    m_impl->push_item();
}
...
void Player::use_pill() {
    m_impl->use_pill();
}

void Player::get_pill(string id) {
    m_impl->get_pill(id);
}

void Player::you_died() {
    m_impl->you_died();
}
