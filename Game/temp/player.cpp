...
void Player::update_self(unsigned long elapsed){
  ...
  if ((x == env->canvas->w() - w() and dx > 0) or (x == 0 and dx < 0)){
  ...
  if ((y == env->canvas->h() - h() and dy > 0) or (y == 0 and dy < 0)){
    change_state(Player::IDLE, m_state);
  }
  ...
  if(posx <= 1 && ( posy >= 280 && posy <= 420) &&
    current_map->current_room->r_left){
      enter_room(current_map->current_room, current_map->current_room->r_left, 1120, posy);
  }
  else if(posx >= 1200 && ( posy >= 280 && posy <= 420) &&
    current_map->current_room->r_right){
      enter_room(current_map->current_room,
      current_map->current_room->r_right, 80, posy);
  }
  else if(posy <= 1  && ( posx >= 600 && posx <= 680) &&
    current_map->current_room->r_top){
      enter_room(current_map->current_room, current_map->current_room->r_top,
        posx, 580);
  }
  else if(posy >= 620  && ( posx >= 600 && posx <= 680) &&
    current_map->current_room->r_bottom){
      enter_room(current_map->current_room,
        current_map->current_room->r_bottom, posx, 80);
  }
  ...
  for(int i = 0; i < aux.size(); i++){
    if(aux[i]->type == "key"){
      if (((posx + 40 > aux[i]->x()) && (posx + 40 < (aux[i]->x() + 32))) &&
        ((posy + 40> aux[i]->y()) && (posy + 40 < (aux[i]->y() + 32)))){
      ...
      if(has_key()){
        if (((posx + 40 > aux[i]->x()) && (posx + 40 < (aux[i]->x() + 80))) &&
          ((posy + 40> aux[i]->y()) && (posy + 40 < (aux[i]->y() + 80)))){
...
void Player::enter_room(Room * anterior, Room * nova, int posx, int posy){
  set_position(posx, posy);
  current_map->enter_room(anterior, nova);
}

void Player::report_event(Event event){
...
void Player::change_state(State to, State){
  m_state = to;
}

short Player::direction() const{
  if(m_left){
    return 0;
  }
  else if(m_up){
...
void Player::pick_key(){
  this->key = true;
}

void Player::drop_key(){
  this->key = false;
}
